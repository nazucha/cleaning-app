 ...
 * https://iair-c.com/manual/index-tsb/index-pnsnc/ から各型番の詳細ページをスクレイピング
 */
    ...
    if (matches) {
      for (const match of matches) {
        ...
        if (model.length >= prefix.length + 2 && model.length <= 25 && !seen.has(model) && model.startsWith(prefixUpper)) {
          if (!model.includes('INDEX') && !model.includes('CATEGORY') && !model.includes('TOSHIBA') && !model.includes('MANUAL')) {
            models.push(model);
            seen.add(model);
          }
        }
...
async function scrapeToshibaFullModels() {
  const fullModelDetails = {};
  const cleaningFunctionData = {};
  const baseUrl = 'https://iair-c.com';
  
  console.log('東芝の完全な詳細型番を取得中...');
  console.log(`処理対象: ${Object.keys(toshibaModelDetails.modelDetails).length}個のプレフィックス\n`);
  
  let totalModels = 0;
  let processedPrefixes = 0;
  
  for (const [prefix, models] of Object.entries(toshibaModelDetails.modelDetails)) {
    processedPrefixes++;
    const modelCount = Array.isArray(models) ? models.length : 0;
    console.log(`[${processedPrefixes}/${Object.keys(toshibaModelDetails.modelDetails).length}] ${prefix} を処理中... (${modelCount}件の型番)`);
    ...
    for (const model of models) {
      ...
      try {
        ...
        for (const category of categories) {
          const url = `${baseUrl}/manual/index-tsb/index-tsb-${category}/${modelLower}/`;
  ...
  const output = {
    manufacturer: '東芝',
    source: 'https://iair-c.com/manual/index-tsb/index-pnsnc/',
    lastUpdated: new Date().toISOString().split('T')[0],
    totalFullModels: totalModels,
    modelDetails: fullModelDetails
  };
  
  const outputPath = path.join(__dirname, '../data/index-tsbFullModelDetails.json');
  fs.writeFileSync(outputPath, JSON.stringify(output, null, 2), 'utf8');
  console.log(`\n結果を ${outputPath} に保存しました`);
  console.log(`合計 ${totalModels} 個の完全な型番を取得しました`);
  
  if (Object.keys(cleaningFunctionData).length > 0) {
    const cleaningOutputPath = path.join(__dirname, '../data/index-tsbCleaningFunctionFromManual.json');
...
if (require.main === module) {
  scrapeToshibaFullModels().catch(console.error);
}

module.exports = { scrapeToshibaFullModels };
