/**
 * 三菱重工の詳細型番を参考サイトから取得するスクリプト
 * https://iair-c.com/manual/mitsubishi-hindustry/index-mhi/ から各プレフィックスのページをスクレイピング
 */
...
// 三菱重工の型番プレフィックス（参考サイトより）
const mitsubishiHeavyPrefixes = [
  'SFM',
  'SKM',
  'SKP',
  'SRF',
...
// メイン処理
async function scrapeMitsubishiHeavyModels() {
  const modelDetails = {};
  const baseUrl = 'https://iair-c.com';
  const indexUrl = 'https://iair-c.com/manual/mitsubishi-hindustry/index-mhi/';
  
  console.log('三菱重工の詳細型番を取得中...');
  ...
  try {
    ...
    for (const category of categories) {
      categoryLinks[category] = `/manual/mitsubishi-hindustry/mhi-${category.toLowerCase()}/index-mhi-${category.toLowerCase()}/`;
    }
    ...
    for (const prefix of mitsubishiHeavyPrefixes) {
      const models = [];
      ...
      for (const [category, url] of Object.entries(categoryLinks)) {
        try {
          ...
          // URLから型番を抽出（例：/manual/mitsubishi-hindustry/mhi-s/sfm-22/ → SFM-22）
          // 三菱重工のURL構造: /manual/mitsubishi-hindustry/mhi-s/sfm-22/
          // matchメソッドを使用してすべてのURLを一度に抽出
          const urlPattern = new RegExp(`/manual/mitsubishi-hindustry/mhi-${category.toLowerCase()}/([a-z0-9\\-]+)/`, 'gi');
          const allMatches = html.match(urlPattern);
          
          if (allMatches) {
            // 各マッチから型番部分を抽出
            for (const match of allMatches) {
              // URLから型番部分を抽出（例：/manual/mitsubishi-hindustry/mhi-s/sfm-22/ → sfm-22）
              const modelMatch = match.match(new RegExp(`mhi-${category.toLowerCase()}/([^/]+)/`, 'i'));
      ...
      const invalidPatterns = [
        'INDEX', 'CATEGORY', 'TAG', 'MITSUBISHI', 'MANUAL', 'HINDUSTRY',
  ...
  const output = {
    manufacturer: '三菱重工',
    source: 'https://iair-c.com/manual/mitsubishi-hindustry/index-mhi/',
    lastUpdated: new Date().toISOString().split('T')[0],
    modelDetails: modelDetails
  };
  
  const outputPath = path.join(__dirname, '../data/mitsubishiHeavyModelDetails.json');
...
if (require.main === module) {
  scrapeMitsubishiHeavyModels().catch(console.error);
}

module.exports = { scrapeMitsubishiHeavyModels };
