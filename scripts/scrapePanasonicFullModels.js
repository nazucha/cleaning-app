/**
 * パナソニックの完全な詳細型番を取得するスクリプト
 * https://iair-c.com/manual/panasonic/index-pnsnc/ から各型番の詳細ページをスクレイピング
 */
...
async function scrapePanasonicFullModels() {
  const fullModelDetails = {};
  const cleaningFunctionData = {};
  const baseUrl = 'https://iair-c.com';
  
  console.log('パナソニックの完全な詳細型番を取得中...');
  console.log(`処理対象: ${Object.keys(panasonicModelDetails.modelDetails).length}個のプレフィックス\n`);
  
  let totalModels = 0;
  let processedPrefixes = 0;
  
  for (const [prefix, models] of Object.entries(panasonicModelDetails.modelDetails)) {
    processedPrefixes++;
    const modelCount = Array.isArray(models) ? models.length : 0;
    console.log(`[${processedPrefixes}/${Object.keys(panasonicModelDetails.modelDetails).length}] ${prefix} を処理中... (${modelCount}件の型番)`);
    ...
    for (const model of models) {
      ...
      try {
        ...
        for (const category of categories) {
          const url = `${baseUrl}/manual/panasonic/panasonic-${category}/${modelLower}/`;
  ...
  const outputPath = path.join(__dirname, '../data/panasonicFullModelDetails.json');
  fs.writeFileSync(outputPath, JSON.stringify(output, null, 2), 'utf8');
  console.log(`\n結果を ${outputPath} に保存しました`);
  console.log(`合計 ${totalModels} 個の完全な型番を取得しました`);
  
  if (Object.keys(cleaningFunctionData).length > 0) {
    const cleaningOutputPath = path.join(__dirname, '../data/panasonicCleaningFunctionFromManual.json');
...
if (require.main === module) {
  scrapePanasonicFullModels().catch(console.error);
}

module.exports = { scrapePanasonicFullModels };
