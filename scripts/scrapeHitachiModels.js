/**
 * 日立の詳細型番を参考サイトから取得するスクリプト
 * https://iair-c.com/manual/hitachi/index-htc/ から各プレフィックスのページをスクレイピング
 */
...
// 日立の型番プレフィックス（参考サイトより）
const hitachiPrefixes = [
  'RAC',
  'RAF',
  'RAJ',
  'RAM',
...
function extractModels(html, prefix) {
  ...
  // URLから型番を抽出（例：/manual/hitachi/ras-22/）
  // より広範囲なURLパターンを検索
  const urlPatterns = [
    new RegExp(`/manual/hitachi/${prefixLower}[0-9a-z\\-]+/`, 'gi'),
    new RegExp(`href=["']/manual/hitachi/${prefixLower}[0-9a-z\\-]+/["']`, 'gi'),
    new RegExp(`href=["']https://iair-c\\.com/manual/hitachi/${prefixLower}[0-9a-z\\-]+/["']`, 'gi'),
  ];
...
async function extractCategoryLinks(html) {
  ...
  // カテゴリーページのリンクパターン（例：/manual/hitachi/htc-r/index-htc-r/）
  const linkPattern = /<a[^>]*href=["']([^"']*\/manual\/hitachi\/htc-([a-z])\/index-htc-[a-z]+\/)["'][^>]*>/gi;
  let match;
  while ((match = linkPattern.exec(html)) !== null) {
    const url = match[1];
    const category = match[2].toUpperCase(); // R
...
// メイン処理
async function scrapeHitachiModels() {
  const modelDetails = {};
  const baseUrl = 'https://iair-c.com';
  const indexUrl = 'https://iair-c.com/manual/hitachi/index-htc/';
  
  console.log('日立の詳細型番を取得中...');
  ...
  try {
    ...
    for (const prefix of hitachiPrefixes) {
      const models = [];
      const prefixLower = prefix.toLowerCase();
      const prefixUpper = prefix.toUpperCase();
      ...
      for (const [category, url] of Object.entries(categoryLinks)) {
        try {
          ...
          // URLから型番を抽出（例：/manual/hitachi/htc-r/hras-a-c/ → HRAS-A-C → RAS-A-C）
          // 日立のURL構造: /manual/hitachi/htc-r/hras-a-c/
          // URLには`h`が前に付いている可能性があるので、両方のパターンを試す
          const urlPatterns = [
            // パターン1: hプレフィックス付き（例：hras-a-c）
            new RegExp(`/manual/hitachi/htc-${category.toLowerCase()}/h${prefixLower}[0-9a-z\\-]+/`, 'gi'),
            // パターン2: プレフィックス直接（例：ras-22）
            new RegExp(`/manual/hitachi/htc-${category.toLowerCase()}/${prefixLower}[0-9a-z\\-]+/`, 'gi'),
          ];
          ...
          for (const urlPattern of urlPatterns) {
            ...
            while ((urlMatch = urlPattern.exec(html)) !== null) {
              const urlPart = urlMatch[0];
              // URLから型番部分を抽出
              let modelMatch = urlPart.match(new RegExp(`h${prefixLower}[0-9a-z\\-]+`, 'i'));
              if (!modelMatch) {
                modelMatch = urlPart.match(new RegExp(`${prefixLower}[0-9a-z\\-]+`, 'i'));
              }
              
              if (modelMatch) {
                let model = modelMatch[0].trim().toUpperCase();
                // hプレフィックスを除去（例：HRAS-A-C → RAS-A-C）
                if (model.startsWith('H' + prefixUpper)) {
      ...
      const invalidPatterns = [
        'INDEX', 'CATEGORY', 'TAG', 'HITACHI', 'MANUAL',
  ...
  const output = {
    manufacturer: '日立',
    source: 'https://iair-c.com/manual/hitachi/index-htc/',
    lastUpdated: new Date().toISOString().split('T')[0],
    modelDetails: modelDetails
  };
  
  const outputPath = path.join(__dirname, '../data/hitachiModelDetails.json');
...
if (require.main === module) {
  scrapeHitachiModels().catch(console.error);
}

module.exports = { scrapeHitachiModels };
