/**
 * シャープの完全な詳細型番を取得するスクリプト
 * https://iair-c.com/manual/index-shp/index-pnsnc/ から各型番の詳細ページをスクレイピング
 */
    ...
    if (matches) {
      for (const match of matches) {
        ...
        if (model.length >= prefix.length + 2 && model.length <= 25 && !seen.has(model) && model.startsWith(prefixUpper)) {
          if (!model.includes('INDEX') && !model.includes('CATEGORY') && !model.includes('SHARP') && !model.includes('MANUAL')) {
            models.push(model);
            seen.add(model);
          }
        }
...
async function scrapeSharpFullModels() {
  const fullModelDetails = {};
  const cleaningFunctionData = {};
  const baseUrl = 'https://iair-c.com';
  
  console.log('シャープの完全な詳細型番を取得中...');
  console.log(`処理対象: ${Object.keys(sharpModelDetails.modelDetails).length}個のプレフィックス\n`);
  
  let totalModels = 0;
  let processedPrefixes = 0;
  
  for (const [prefix, models] of Object.entries(sharpModelDetails.modelDetails)) {
    processedPrefixes++;
    const modelCount = Array.isArray(models) ? models.length : 0;
    console.log(`[${processedPrefixes}/${Object.keys(sharpModelDetails.modelDetails).length}] ${prefix} を処理中... (${modelCount}件の型番)`);
    ...
    for (const model of models) {
      ...
      try {
        ...
        for (const category of categories) {
          const url = `${baseUrl}/manual/index-shp/index-shp-${category}/${modelLower}/`;
  ...
  const output = {
    manufacturer: 'シャープ',
    source: 'https://iair-c.com/manual/index-shp/index-pnsnc/',
    lastUpdated: new Date().toISOString().split('T')[0],
    totalFullModels: totalModels,
    modelDetails: fullModelDetails
  };
  
  const outputPath = path.join(__dirname, '../data/index-shpFullModelDetails.json');
  fs.writeFileSync(outputPath, JSON.stringify(output, null, 2), 'utf8');
  console.log(`\n結果を ${outputPath} に保存しました`);
  console.log(`合計 ${totalModels} 個の完全な型番を取得しました`);
  
  if (Object.keys(cleaningFunctionData).length > 0) {
    const cleaningOutputPath = path.join(__dirname, '../data/index-shpCleaningFunctionFromManual.json');
...
if (require.main === module) {
  scrapeSharpFullModels().catch(console.error);
}

module.exports = { scrapeSharpFullModels };
