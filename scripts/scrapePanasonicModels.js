/**
 * パナソニックの詳細型番を参考サイトから取得するスクリプト
 * https://iair-c.com/manual/panasonic/index-pnsnc/ から各プレフィックスのページをスクレイピング
 */
...
// パナソニックの型番プレフィックス（参考サイトより）
const panasonicPrefixes = [
  'CS'
];
...
function extractModels(html, prefix) {
  ...
  // URLから型番を抽出（例：/manual/panasonic/cs-22/）
  // より広範囲なURLパターンを検索
  const urlPatterns = [
    new RegExp(`/manual/panasonic/${prefixLower}[0-9a-z\\-]+/`, 'gi'),
    new RegExp(`href=["']/manual/panasonic/${prefixLower}[0-9a-z\\-]+/["']`, 'gi'),
    new RegExp(`href=["']https://iair-c\\.com/manual/panasonic/${prefixLower}[0-9a-z\\-]+/["']`, 'gi'),
  ];
...
async function extractCategoryLinks(html) {
  ...
  // カテゴリーページのリンクパターン（例：/manual/panasonic/pnsnc-c/index-pnsnc-c/）
  const linkPattern = /<a[^>]*href=["']([^"']*\/manual\/panasonic\/pnsnc-([a-z])\/index-pnsnc-[a-z]+\/)["'][^>]*>/gi;
  let match;
  while ((match = linkPattern.exec(html)) !== null) {
    const url = match[1];
    const category = match[2].toUpperCase(); // C
...
// メイン処理
async function scrapePanasonicModels() {
  const modelDetails = {};
  const baseUrl = 'https://iair-c.com';
  const indexUrl = 'https://iair-c.com/manual/panasonic/index-pnsnc/';
  
  console.log('パナソニックの詳細型番を取得中...');
  ...
  try {
    ...
    for (const prefix of panasonicPrefixes) {
      const models = [];
      const prefixLower = prefix.toLowerCase();
      const prefixUpper = prefix.toUpperCase();
      ...
      for (const [category, url] of Object.entries(categoryLinks)) {
        try {
          ...
          // URLから型番を抽出（例：/manual/panasonic/pnsnc-c/cs-22/ → CS-22）
          // パナソニックのURL構造: /manual/panasonic/pnsnc-c/cs-22/
          const urlPatterns = [
            new RegExp(`/manual/panasonic/pnsnc-${category.toLowerCase()}/${prefixLower}[0-9a-z\\-]+/`, 'gi'),
            new RegExp(`/manual/panasonic/panasonic-${category.toLowerCase()}/${prefixLower}[0-9a-z\\-]+/`, 'gi'),
          ];
      ...
      const invalidPatterns = [
        'INDEX', 'CATEGORY', 'TAG', 'PANASONIC', 'MANUAL',
  ...
  const output = {
    manufacturer: 'パナソニック',
    source: 'https://iair-c.com/manual/panasonic/index-pnsnc/',
    lastUpdated: new Date().toISOString().split('T')[0],
    modelDetails: modelDetails
  };
  
  const outputPath = path.join(__dirname, '../data/panasonicModelDetails.json');
...
if (require.main === module) {
  scrapePanasonicModels().catch(console.error);
}

module.exports = { scrapePanasonicModels };
